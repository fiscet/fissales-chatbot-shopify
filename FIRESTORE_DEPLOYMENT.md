...
```bash
docker build -t gcr.io/YOUR_PROJECT_ID/villma-chatbot .
```

3. **Push to Google Container Registry:**
```bash
docker push gcr.io/YOUR_PROJECT_ID/villma-chatbot
```

4. **Deploy to Cloud Run:**
```bash
gcloud run deploy villma-chatbot \
  --image gcr.io/YOUR_PROJECT_ID/villma-chatbot \
  --platform managed \
  --region us-central1 \
